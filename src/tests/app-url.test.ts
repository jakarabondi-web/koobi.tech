...
describe("appUrl", () => {
  it("prefers explicit configuration, since a custom domain can't be inferred", () => {
    process.env.NEXT_PUBLIC_APP_URL = "https://trainora.ai";
    process.env.VERCEL_PROJECT_PRODUCTION_URL = "proj.vercel.app";
    expect(appUrl()).toBe("https://trainora.ai");
  });
  ...
  it("falls back to Vercel's production URL, not localhost", () => {
    ...
    process.env.VERCEL_PROJECT_PRODUCTION_URL = "trainora.vercel.app";
    expect(appUrl()).toBe("https://trainora.vercel.app");
  });

  it("prefers the stable production URL over the per-deployment one", () => {
    process.env.VERCEL_PROJECT_PRODUCTION_URL = "trainora.vercel.app";
    process.env.VERCEL_URL = "trainora-abc123.vercel.app";
    expect(appUrl()).toBe("https://trainora.vercel.app");
  });

  it("uses the deployment URL when that's all there is", () => {
    process.env.VERCEL_URL = "trainora-abc123.vercel.app";
    expect(appUrl()).toBe("https://trainora-abc123.vercel.app");
  });
